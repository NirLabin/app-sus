...
            <li class="note" v-for="note in notes.pinned" :key="note.id" 	
                :style="{backgroundColor : note?.style?.bgc}"  >
                <note-preview :note="note" @edit="edit" @pin="pin" 	     
                 @color="changeColor" @duplicate="duplicate" @remove="remove" @todo="todo"  @addTodo="addTodo"  @save="save" @send="send"/>
            </li> 
        </ul>
        <ul v-if="notes && notes.unPinned.length" class="note-list">
            <li class="note" v-for="note in notes.unPinned" :key="note.id"  
               :style="{backgroundColor : note?.style?.bgc}" >
                <note-preview :note="note" @edit="edit" @pin="pin" 
                @color="changeColor" @duplicate="duplicate" @remove="remove" @todo="todo" @addTodo="addTodo"  @save="save" @send="send"/>
            </li> 
